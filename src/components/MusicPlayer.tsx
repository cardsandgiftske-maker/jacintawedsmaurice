  ...
  return (
    <>
      ...
      <iframe
        ...
        width="1"
        height="1"
        src={`https://www.youtube.com/embed/QgaTQ5-XfMM?enablejsapi=1&autoplay=0&controls=0&loop=1&playlist=QgaTQ5-XfMM&origin=${typeof window !== 'undefined' ? encodeURIComponent(window.location.origin) : ''}`}
        ...
        {shouldPlay && (
          ...
          >
            <div className="flex items-center gap-2 bg-white/95 backdrop-blur-md border border-stone-200/80 rounded-full p-1.5 shadow-[0_8px_30px_rgb(0,0,0,0.06)] hover:shadow-[0_10px_35px_rgb(0,0,0,0.1)] hover:border-stone-300 transition-all group">
              ...
              <button
                onClick={togglePlay}
                className="w-10 h-10 rounded-full bg-[#87434E]/10 border border-[#87434E]/20 flex items-center justify-center text-[#87434E] hover:bg-[#87434E] hover:text-white transition-all cursor-pointer relative overflow-hidden group/btn"
              ...
              <div className="flex items-center gap-1.5 pr-3 pl-1 font-serif text-xs font-medium tracking-tight text-stone-700 select-none">
                ...
                <button
                  onClick={toggleMute}
                  className="p-1 text-stone-500 hover:text-[#87434E] active:scale-90 transition-all cursor-pointer"
                  title={isMuted ? "Unmute Music" : "Mute Music"}
                >
                  {isMuted ? (
                    <VolumeX className="w-4 h-4 text-rose-500" />
                  ) : (
                    <Volume2 className="w-4 h-4 text-[#58735B]" />
                  )}
                </button>
              </div>
            </div>
